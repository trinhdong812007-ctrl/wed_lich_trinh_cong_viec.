...
├─ .gitignore ───────► Loại trừ __pycache__/, *.db, instance/, .venv/, .env, IDE files...
├─ .last_update ─────► Timestamp phục vụ API `/api/last-update` (tự sinh bởi `touch_last_update()`)
│
│  ┌──────────────────────────────────────────────────────────────┐
│  │                     MÃ NGUỒN CHÍNH (app/)                    │
│  ├──────────────────────────────────────────────────────────────┤
...
│  │  │
│  │  ├─ base.html ────► layout khung chung (sidebar, topbar) — mọi template con đều extends
│  │  ├─ auth/ ────────► login.html · register.html
│  │  ├─ dashboard/ ───► lich_trinh.html · reports.html · dynamic_page.html · dashboard.html*
...
│
├─ instance/ ◀─────── DỮ LIỆU RUNTIME (không commit lên git, tự tạo khi chạy `create_app()`)
│  └─ database.db ───► CSDL SQLite chính (đường dẫn khai trong `config.py`)
│
...
**Ghi chú đọc sơ đồ:** `─►` = trỏ tới chức năng/mô tả · `app/` là package trung tâm gồm 4 tầng: **Models → Routes → Services → Templates** · `*` = file tồn tại trong mã nguồn nhưng hiện **chưa được route nào `render_template()` gọi tới** (`dashboard.html` — có thể là bản nháp/dự phòng cho trang tổng quan riêng).
...
| `requirements.txt` | `Flask==3.0.3`, `Flask-SQLAlchemy==3.1.1`, `SQLAlchemy==2.0.36`, `Flask-Login==0.6.3`, `Flask-Migrate==4.0.7`, `Werkzeug==3.0.3`, `gunicorn` (không ghim version), `openpyxl==3.1.5`. |
| `Procfile` | `web: gunicorn app:app` — dùng khi triển khai lên Render. |
| `generate_keys.py` | Script CLI sinh Key kích hoạt ngẫu nhiên dạng `XXXX-XXXX-XXXX` (dùng `uuid.uuid4().hex`), lưu vào bảng `activation_keys`. |
| `employees_sample.xlsx`, `tasks_sample.xlsx` | File mẫu dùng cho chức năng tải template & nhập dữ liệu. |
| `.gitignore` | Loại trừ `__pycache__/`, `*.py[cod]`, `*.db`, `instance/`, `.venv/`/`venv/`/`env/`, `.env`, `.vscode/`/`.idea/`, `*.log`, `.DS_Store`. |
| `.last_update` | File timestamp (ISO format) được `touch_last_update()` cập nhật mỗi khi có thay đổi dữ liệu; đọc bởi API `/api/last-update` để client polling trạng thái mới nhất. |
...
| `dashboard/` | `lich_trinh.html`, `reports.html`, `dynamic_page.html`, `dashboard.html`* | Lịch làm việc tuần, báo cáo, trang động; `dashboard.html` extends `base.html` nhưng hiện chưa có route nào render tới (file dự phòng). |
